...
// ===== Package System (3/5/10 회기) =====
const PACKAGES = {
  starter: { id: 'starter', name: '스타터', sessions: 3, description: '빠른 문제 해결, 단일 주제 집중' },
  basic: { id: 'basic', name: '베이직', sessions: 5, description: '1-2개 주제 집중, 루틴 정착' },
  premium: { id: 'premium', name: '프리미엄', sessions: 10, description: '3개 주제 모두 다룸, 충분한 반복' }
};

// ===== Coaching Topics (주제별 점수 관리) =====
const CoachingTopics = {
  // 주제 가져오기
  getTopics(coacheeId) {
    const goal = DB.get('goalAgreements').find(g => g.coacheeId === coacheeId);
    if (!goal || !goal.topics) return [];
    return goal.topics;
  },

  // 주제 점수 업데이트
  updateTopicScore(coacheeId, topicIndex, currentScore) {
    const goal = DB.get('goalAgreements').find(g => g.coacheeId === coacheeId);
    if (!goal || !goal.topics) return null;

    if (goal.topics[topicIndex]) {
      goal.topics[topicIndex].currentScore = currentScore;
      goal.topics[topicIndex].updatedAt = new Date().toISOString();
      DB.upsert('goalAgreements', goal);
    }
    return goal;
  },

  // 주제별 점수 히스토리 가져오기
  getScoreHistory(coacheeId, topicIndex) {
    const reflections = DB.get('reflections').filter(r => r.coacheeId === coacheeId);
    return reflections
      .filter(r => r.topicScores && r.topicScores[topicIndex] !== undefined)
      .map(r => ({
        sessionNumber: parseInt(r.sessionId?.split('_')[2]) || 0,
        score: r.topicScores[topicIndex],
        date: r.createdAt
      }))
...
// ===== Execution Records (실행 기록) =====
const ExecutionRecords = {
  // 실행 기록 추가
  add(coacheeId, record) {
    const item = {
      id: genId(),
      coacheeId,
    ...
    return DB.addItem('executionRecords', item);
  },

  // 피코치의 실행 기록 가져오기
  getByCoachee(coacheeId, limit = 10) {
    return DB.get('executionRecords')
      .filter(r => r.coacheeId === coacheeId)
  ...
  // 완료 처리
  toggle(recordId) {
    const record = DB.getItem('executionRecords', recordId);
    if (record) {
      record.completed = !record.completed;
      DB.upsert('executionRecords', record);
    }
    return record;
  }
};

// ===== Session Tasks (다음 회기까지 과제) =====
const SessionTasks = {
  // 과제 추가
  add(coacheeId, sessionNumber, task) {
    const item = {
      id: genId(),
      coacheeId,
      sessionNumber,
    ...
    return DB.addItem('sessionTasks', item);
  },

  // 현재 과제 가져오기 (미완료)
  getPending(coacheeId) {
    return DB.get('sessionTasks')
      .filter(t => t.coacheeId === coacheeId && !t.completed)
      .sort((a, b) => a.sessionNumber - b.sessionNumber);
  },

  // 완료 처리
  complete(taskId) {
    const task = DB.getItem('sessionTasks', taskId);
    if (task) {
      task.completed = true;
      task.completedAt = new Date().toISOString();
      DB.upsert('sessionTasks', task);
    }
    return task;
  },

  // 특정 회기 과제 가져오기
  getBySession(coacheeId, sessionNumber) {
    return DB.get('sessionTasks')
      .filter(t => t.coacheeId === coacheeId && t.sessionNumber === sessionNumber);
  }
};
...
function seedDemoData() {
  ...
  demos.forEach((d, i) => {
    ...
    // 패키지 타입 결정
    const packageTypes = ['starter', 'basic', 'premium'];
    const pkgType = packageTypes[i % 3];
    const pkgSessions = PACKAGES[pkgType].sessions;
    ...
    DB.addItem('goalAgreements', {
      ...
      topics: [
        {
          title: '공부 시작 행동 전환',
          desiredOutcome: '스카에 도착한 뒤 실제 공부 행동으로 전환할 수 있다',
          startScore: 0,
          currentScore: Math.min(d.progress + 1, 5),
          targetScore: 5
        },
        {
          title: '전자기기 방해 관리',
          desiredOutcome: '카톡, 유튜브 대신 공부 행동을 먼저 선택할 수 있다',
          startScore: 0,
          currentScore: Math.min(d.progress, 5),
          targetScore: 5
        },
        {
          title: '복귀 행동 만들기',
          desiredOutcome: '공부가 끊겨도 다시 돌아올 수 있다',
          startScore: 2,
          currentScore: Math.min(d.progress + 2, 5),
          targetScore: 5
        }
      ]
    });
    ...
    // 실행 기록 추가
    for (let r = 0; r < Math.min(d.progress * 2, 6); r++) {
      const recordDate = new Date(Date.now() - (6 - r) * 86400000).toISOString().split('T')[0];
      DB.addItem('executionRecords', {
        id: `exec_${id}_${r}`,
        coacheeId: id,
        date: recordDate,
        content: ['오전 10분 인강 성공', '핸드폰 보관함 활용', '커피 타기 전 인강 듣기', '워드 작성 10분 완료', '타임워치 사용', '복귀 문장 사용'][r % 6],
        completed: Math.random() > 0.3,
        sessionNumber: Math.ceil((r + 1) / 2),
        createdAt: new Date(recordDate).toISOString()
      });
    }

    // 과제 추가
    if (d.progress < 4) {
      DB.addItem('sessionTasks', {
        id: `task_${id}_1`,
        coacheeId: id,
        sessionNumber: d.progress,
        content: '오후 루틴 2번 실험해보기',
        completed: false,
        createdAt
      });
      DB.addItem('sessionTasks', {
    ...
    for (let s = 1; s <= Math.min(d.progress, pkgSessions); s++) {
